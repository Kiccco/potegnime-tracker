...
static mem_node_t* add_node_to_tree(mem_node_t* rootNode, mem_node_t* node);
static mem_node_t* node_remove(mem_node_t* root, mem_node_t* node);
static mem_node_t* find_min_node(mem_node_t* node);


void mem_pool_init(mem_pool_t* pool, size_t poolSize, size_t dataSize) {
    pool->pool = malloc(poolSize * dataSize);
...
void mem_pool_add_node(mem_pool_t* pool, mem_node_t* node) {
    ...
    add_node_to_tree(pool->root_node, node);
}
...
mem_node_t* mem_pool_alloc_node(mem_pool_t* pool, U32 key) {
    ...
    pool->root_node = add_node_to_tree(pool->root_node, node);
...
mem_node_t* mem_pool_find_node(mem_pool_t* pool, U32 key) {
    ...
    mem_node_t* temp = pool->root_node;
...
void mem_pool_free_node(mem_pool_t* pool, mem_node_t* node) {
    ...
    //refresh tree
    node_remove(pool->root_node, node);
...
static mem_node_t* add_node_to_tree(mem_node_t* root, mem_node_t* node) {

    if (root == NULL) 
        return node;
    
    if (root->key > node->key) {
        root->left = add_node_to_tree(root->left, node);
    }
    else if (root->key < node->key) {
        root->right = add_node_to_tree(root->right, node);
    }
    else {
        return root;
    }
...
static mem_node_t* node_remove(mem_node_t* root, mem_node_t* node) {
    ...
    if (node->key < root->key) {
        root->left = node_remove(root->left, node);
    }
    else if (node->key > root->key) {
        root->right = node_remove(root->right, node);
    }
    else {
        if (root->left == NULL) {
            return root->right;
        ...
        root->right = node_remove(root->right, temp);
    }
